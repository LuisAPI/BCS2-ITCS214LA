    ...
    def push(self, data):
        if not self.is_full():
            if data not in self.stack:
                ...
                return f"Success! “{data}” added to stack!"
            else:
                # element is already in stack
                return f"Error! “{data}” already in stack!"
        else:
            # Stack is full or overflow
            return f"Stack Overflow! “{data}” cannot be added to stack!"
    
    def display_elements(self):
        if self.is_empty():
            print("Stack is empty.")
        else:
            print("Elements in the stack:", self.stack)
        print("")
...
### Print error when stack is empty
